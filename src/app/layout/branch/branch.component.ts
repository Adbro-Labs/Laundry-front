...
export class BranchComponent implements OnInit {
  ...
  query = "";
  constructor(private dialog: MatDialog, private branch: BranchService) { }

  ngOnInit(): void {
    this.getAllCustomers();
  }
  showCustomerDialog() {
    this.dialog.open(AddBranchComponent, { disableClose: true, width: '400px'}).afterClosed().subscribe(data => {
      this.getAllCustomers();
    });
  }
  getAllCustomers() {
